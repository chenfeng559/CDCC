...
from utils_DTC import plot_confusion_martix, print_cluster_info,evaluation_cls_metric,evaluation_clu_metric,plot_confusion_martix,get_logger
...
def train_ClusterNET_with_fake(model, trainloader_list, optimizer_clu, epoch, args, verbose):
    ...
    for i in range(len(trainloader_list)):
        ...
        for batch_idx, (inputs, labels, anomaly_labels, seq_anomaly_labels) in enumerate(trainloader):
            ...
            total_loss = loss_mse + loss_KL*args.gamma1+l2_regularization*args.gamma2+loss_cls*args.gamma3
            total_loss.backward()
            optimizer_clu.step()
...
def test_function(args, model, testloader_list):
    ...
    # 打印聚类信息
    print_cluster_info(preds_list, gt_list, logger)
    # 打印聚类结果：acc、f1、轮廓系数
    print_test_info(preds_list, gt_list, anomaly_label_list, seq_anomaly_label_list, z_list, errors_list)
    # confusion martix
    plot_confusion_martix(preds_list, gt_list, picture_name='experiment/DTC-Cluster-exp/SAD360+NATOPS400_ClusterNum6.png')
    return preds_list,gt_list,anomaly_label_list,seq_anomaly_label_list,z_list,errors_list


def print_test_info(preds_list,gt_list,anomaly_label_list,seq_anomaly_label_list,z_list,errors_list):
    '''print test_metric'''
    #roc_auc = evaluation_cls_metric('roc_auc',preds_list,gt_list)
    acc = evaluation_cls_metric('acc',preds_list,gt_list)
    f1 = evaluation_cls_metric('f1',preds_list,gt_list)
...
if __name__ == "__main__":
    ...
    args.path_weights_ae = os.path.join(path_weights, "autoencoder_weight_test.pth")
    args.path_weights_main = os.path.join(path_weights, "full_model_weigths_test.pth")
    args.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    args.batch_size = 16
    args.drop_last = True

    logger = get_logger('experiment_log/DTC_Cluster-exp/exp_classification.log')
    ...
    data_list = [get_data(args, dataset, pad_length[i], clamp_length[i], label_modify[i], concat_test=concat_test[i], anomaly=True) for i, dataset in enumerate(datasets)]
    ...
    model_test.load_state_dict(torch.load(args.path_weights_main))

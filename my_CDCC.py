...
def parse_MTS_data(data_dir, scale, window_size, step):
    ...
    print("after spilt X.shape",{X.shape})
    print("spilt y.shape",{y.shape})
    return (X, y)
...
def parse_csvdata(data_dir, scale, window_size=[24,1], step=10):
    array_x_list = []
    array_y_list = []
    ...
    array_x_list.append(X)
    array_y_list.append(y)
    X = split_sample(array_x_list, window_size, step)
    Y = split_sample(array_y_list, [window_size[0], 1], step)
    print(f"X shape before split_sample: {X.shape}")
...
def main():
    ...
    print("### CNNoutput_channel:", algorithm.CNNoutput_channel)
    ...
    ds = parse_csvdata(data_dir,scale=True)
    # sys.exit()
    print()
    # if len(ds) > 0:

...
class CDCC(nn.Module):
    ...
    def forward(self, x_in, x_in_aug, mode='t'):
        ...
        if mode == 'f':
            ...
            # print("推理过程，h_x.shape :",{h_x.shape}) 
            # (256, 320)
            
        
            z_i_x = normalize(self.instance_projector_f(h_x), dim=1)  
            # z_i(batch*class_num) 对两种数据增强方式的列向量进行实例级对比聚类，并归一化
            z_c_x = self.cluster_projector_f(h_x)

            z_i_aug = normalize(self.instance_projector_f(h_aug), dim=1)  
            # z_i(batch*class_num) 对两种数据增强方式的列向量进行实例级对比聚类，并归一化

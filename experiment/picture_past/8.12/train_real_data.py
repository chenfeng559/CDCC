...
from utils import test_metric, save_preds,print_cluster_info,test_metric_cluster,print_error_info,compute_class_specific_accuracy,print_f1_f1pa,compute_confusion_martix
    ...
    for epoch in range(args.epochs_ae):
        ...
        for i in range(len(trainloader_list)):
            ...
            for batch_idx, (inputs, _, anomaly_labels,_) in enumerate(trainloader):
                ...
                loss_mse = loss_ae(inputs[nomaly_indices], x_reconstr[nomaly_indices])+l2_regularization*args.gamma2
                loss_mse.backward()
...
def get_data(args, dataset_name, channels):
    ...
    return get_loader_np_train_test(args, dataset_name, True, [55, channels], 55, anomaly=False,ano_sample_rate=0.5)
...
if __name__ == "__main__":
    ...
    preds_list, gt_list, anomaly_label_list, seq_anomaly_label_list, z_list, errors_list,class_pred_pro_list,class_label_list = test_function(args,model,testloader_list)
